...
def inverse_to_image(args, images, X_train, inv_whitening_mat):
    ...
    for i, (image, beta) in enumerate(images):
        ...
        lower, upper = np.min(origin_image), np.max(origin_image)
        origin_image = (origin_image - lower) / (upper - lower)
...
def rbf_most_similar(Z, X, device):
    ...
    similarity = torch_rbf_kernel(Z, X, 0.1)
...
def plot_comparison_diagram(args, uploader_id, rbf_market: DummyMarket, ntk_market: DummyMarket):
    ...
    X_train_transformed, _, _, _ = data_downloader.get_cifar10(output_channels=3)
    ...
    most_similar_idxes = rbf_most_similar(np.stack(rbf_images), X_train, device)
    for i, idx in enumerate(most_similar_idxes):
        axs[1][i].imshow(np.transpose(X_train[idx], [1,2,0]))
